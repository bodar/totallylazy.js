...
import {extraDelimiters, mappingParser, namedRegexParser, or, Parser, preProcess, TypeSafeParser} from "../parsing";
...
export function numberParser(decimalSeparatorOrLocale?: AllowedDecimalSeparators | Locale, locale: Locale = 'en'): Parser<number> {
    ...
    if (isDecimalSeparator(decimalSeparatorOrLocale)) return new TypeSafeParser(new NumberParser(ignore => decimalSeparatorOrLocale, locale));
